...
#Изменен output на vcf (требуется при crossmap)
LIFT_OVERED38="$OUT_DIR/lift38_merged_excl_stop_normalized.vcf"
...
#Liftover tool изменен с picard  на CrossMap
CrossMap vcf \
...
# Добавлена сортировка и индексация
SORTED_VCF="$OUT_DIR/lift38_sorted.vcf.gz"
bcftools sort "$LIFT_OVERED38" -Oz -o "$SORTED_VCF"
tabix -p vcf "$SORTED_VCF"

echo "STEP 6: PHARMCAT PREPROCESSING"

#Инпут изменен на сортированный vcf.gz
...
#Инпут изменен на сортированный vcf.gz
~/pharmcat/pharmcat_vcf_preprocessor -vcf "$SORTED_VCF" --output-dir "$PROC_ALL_DIR" 2>> "$LOG_FILE"
